...
f_values = [0.05, 0.08, 0.095]  # Different fractions of live cells for the three simulations
...
def update_grid(grid):
    ...
    for i in range(grid_size):
        for j in range(grid_size):
            ...
            # Apply the rules with A dependancies 
            if A >= 0 and A <= 1:
                new_grid[i, j] = 0  # Dead
            elif A > 1 and A <= 2:
                new_grid[i, j] = (np.sqrt(2) + 1)*(A-1)/ 2  # Semi-live (gray)
            elif A > 2 and A <= 3:
                new_grid[i, j] = (np.sqrt(2) + 1)*(A-2)/ 3  # Semi-live (gray)
            elif A > 3 and A <= 4:
                new_grid[i, j] = (np.sqrt(2) + 1)*(A-3)/4  # Live
            else:
                new_grid[i, j] = 0  # Dead
